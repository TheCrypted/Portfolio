    ...
    return (
        <div className="w-full h-40 overflow-hidden flex flex-col">
            <div className="grid grid-cols-[60%_30%_10%] w-full h-full">
                <div onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)} className="hover:blur-sm transition-all text-8xl pb-4 text-white flex items-end">
                    {children}
                </div>
                ...
                <div className="flex items-end pb-8 text-6xl justify-end">
                    ...
                    <div style={{zIndex: 3 , backgroundImage: `url("${resources[parseInt(ind)]}")`, transform: `rotate(${rot}deg)`}} className={` animate-float bg-cover bg-no-repeat bg-center shadow-xl z-20 absolute ${hovering ? "right-80" : "right-[-500px]"} w-80 h-80 rounded-2xl duration-1000 transition-all ${hovering ? "opacity-100" : "opacity-0"} rotate-${rot} bg-black shadow-x`}>
                </div>
            </div>
